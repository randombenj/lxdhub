...
export class ImageService {
  ...
  async findByRemote(
    ...
    return await this.imageRepository.findByRemote(
      remote.id,
      pagination,
      search
    );
  ...
  async cloneImage(
    ...
    const sourceRemote = await this.remoteService.findById(
      cloneImageDto.sourceRemoteId
    );
    if (!sourceRemote) throw new NotFoundException('Source Remote not found');

    const destinationRemote = await this.remoteService.findById(
      cloneImageDto.destinationRemoteId
    );
    if (!destinationRemote)
      throw new NotFoundException('Destination Remote not found');
